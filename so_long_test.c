...
/*   Updated: 2025/02/09 16:44:06 by hwahmane         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
// 		{
// 			if (line[i] == '1')
// 				create_element_image(data, '1', data.elem.map_x,
						data.elem.map_y);
						// 			else if (line[i] == 'C')
						// 				create_element_image(data, 'C',
												data.elem.map_x,
						data.elem.map_y);
												// 			else if (line[i] == 'E')
												// 				create_element_image(data,
																		'E',
												data.elem.map_x,
						data.elem.map_y);
																		// 			else if (line[i] == 'P')
						...
						data.elem.map_y);
										...
										keysym);
														...
														// 	{
														// 		data->img.step_top_bottum
														+= 100;
														// 		mlx_clear_window(data->mlx,
														data->mlx_win);
														// 		draw_map(*data,
														data->img.step_right_left,
										data->img.step_top_bottum);
														// 	}
										...
										data->img.step_top_bottum);
													...
													void *map_len(mlx_data *data)
													{
														int fd;
														char *line;

														fd = open("./maps/map.ber",
																	O_RDONLY);
														line = get_next_line(fd);
														data->map.map_x_len = strlen(line)
															- 1;
													...
													char **map_to_array(mlx_data *data)
													{
														int fd;
														int i;
														int j;
														...
														fd = open("./maps/map.ber",
																	O_RDONLY);
														line = get_next_line(fd);
														data->map.array = malloc((data->map.map_y_len
																	+ 1)
																* sizeof(char *));
														i = 0;
													...
													int main(int ac, char **av)
													{
														...
														if (check_element_map() == 0)
															return (0);
														data.map.array = map_to_array(&data);
									...
									HEIGHT);
										...
										// 		pixel_pos = y
										*data.img.line_len
											+ x *(data.img.bits_per_pixels / 8);
										// 		*((unsigned int *)(data.img.img_pixels_ptr
										+ pixel_pos)) = 0xFF00FF;
										// 		x++;
										// 	}
										// 	y++;
